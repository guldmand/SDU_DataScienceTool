...
# Optional: uvloop på *nix for hurtigere event loop
try:
...
# Optional Redis (fallback til in-memory hvis ikke tilgængelig)
try:
...
class ApiClient:
    """
    Async HTTP-klient med:
    - HTTP/2 + connection pooling (httpx)
    - Rate limiting (aiolimiter)
    - Retries m. backoff (tenacity)
    - TTL-cache (Redis hvis tilgængelig; ellers in-memory)
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.client = httpx.AsyncClient(
            http2=True, timeout=timeout_s, headers=headers or {}
        )
    ...
    async def post_json(
        self, path: str, *, payload=None, headers=None, use_cache=False
    ):
        return await self._request(
            "POST",
            self._mkurl(path),
